...
function EnterTimesheet({setPage}) {
    ...
    const handleSubmit = async () => {
        ...
        // submit entry
        try {
            await axios.post(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/appendtimesheet`, entryObject);
            // stop spinner
    ...
    useEffect(()=>{
        async function getSupportLists() {
            // tasks
            try {
                const tasksArrays = await axios.get(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/supportlists/tasks`);
                let incomingTasks = [];
            ...
            // current jobs
            try {
                const currentJobsArrays = await axios.get(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/supportlists/currentjobs`);
