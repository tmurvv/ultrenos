...
function ViewTimesheets() {
    ...
    async function handleDelete(delId) {
        ...
        try {
            ...
            // Submit Entry
            await axios.post(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/deletetimesheet`, {delid: delId});
            // set environment
            setPage('RefreshView');
            // stop spinner
    ...
    useEffect(()=>{
        ...
        async function getEntries() {
            try {
                // get entries
                const res = await axios.post(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/viewtimesheetsbyuser`, {userid: user.email});
