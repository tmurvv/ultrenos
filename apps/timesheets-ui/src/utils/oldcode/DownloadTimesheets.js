...
function DownloadTimesheets({setPage}) {
    ...
    useEffect(()=>{
        const numSheets = async () => {
            const res = await axios.get(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/numtimesheets`);
            ...
            <div className="form-container" style={{marginTop: '50px'}}>
                <div style={{width: '100%', display: 'flex', justifyContent: 'center'}}>
                    <button type='button' className="submit-btn login-signup-title" style={{boxShadow: '3px 3px 3px lightgrey', width: '150px', margin: 'auto'}}>
                        <a href={`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/downloadtimesheets`} onClick={()=>setNumSheets('Refresh window for number of ')} style={{textDecoration: 'none', fontFamily: 'sans-serif', letterSpacing: '2px', fontSize: '14px', color: 'white'}}>Download Timesheets</a>
                    </button>
                </div>
            </div>
            <LoginSignupCSS />
