...
function ResetPassword(props) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // reset password
            await axios.post(`https://timesheets-staging-api.ultrenos.ca/api/v1/ultrenostimesheets/users/resetpassword`, {useremail: props.useremail, newpassword: userLogin.newpassword});
            // stop spinner
    ...
    return (
       <>
        <div style={{padding: '70px'}} sx={sx.'loginReset-signupReset-container'>
            <PageTitle maintitle='Reset Password' subtitle='Passwords must be at least 8 characters' />
            <Spinner />
            <div style={{transform: 'none', marginTop: '50px'}} sx={sx."login-signup l-attop" id="login">
                <div sx={sx."login-signup-title">
                    Reset Password
                </div>
                <form>
                    ...
                    <button type='button' onClick={()=>handleSubmit()} sx={sx."submit-btn login-signup-title">
                        Submit
                    </button>
                </form>
            </div>
